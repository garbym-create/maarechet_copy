  ...
  document.getElementById('print-sheets').innerHTML = chunks.map((def, pi) => {
    ...
    h += '<table class="sheet-table"><tr><th class="w1">יום</th><th class="w1">שעה</th>' +
      chunk.map(c => '<th>' + esc(c.name) +
        ...
        '</th>').join('') + '</tr>';
    for (const day of DAYS) {
      // בסדין הכיתות מדפיסים עד שעה 7 לכל היותר; סדין המורים — מלא
...
/* ===== מסך פתיחה — נוהל אבטחה, הסבר על המערכת והסרת אחריות (מופיע בכל כניסה) ===== */
function initWelcome() {
  const screen = document.getElementById('welcome-screen');
  screen.hidden = false; // מוצג בכל טעינה — נוהל האבטחה חייב להיראות
  document.getElementById('welcome-close').addEventListener('click', () => { screen.hidden = true; });
